(() => {
if (window.__PLG_SIS_V081__) return;
window.__PLG_SIS_V081__ = true;
...
browser.runtime.onMessage.addListener(message=>{
  if(message?.type==="PLG_READ_SIS_PART"){
    try{return Promise.resolve({ok:true,...parseCurrentSisPart()});}
    catch(error){return Promise.resolve({ok:false,status:error.message});}
  }
});
})();