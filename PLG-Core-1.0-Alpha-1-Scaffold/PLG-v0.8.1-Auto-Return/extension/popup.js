...
async function returnToVerifiedPart(jobId,partId){
  const targetUrl=`http://127.0.0.1:8000/jobs/${jobId}#part-${partId}`;
  const tabs=await browser.tabs.query({});
  const plgTab=tabs.find(tab=>
    tab.url?.startsWith(`http://127.0.0.1:8000/jobs/${jobId}`) ||
    tab.url?.startsWith(`http://localhost:8000/jobs/${jobId}`)
  );
  if(plgTab?.id){
    await browser.tabs.update(plgTab.id,{url:targetUrl,active:true});
    if(plgTab.windowId){
      await browser.windows.update(plgTab.windowId,{focused:true});
    }
  }else{
...
$("importPart").onclick=async()=>{
  ...
  setStatus("Verified. Returning to the updated PLG part…");
  await new Promise(resolve=>setTimeout(resolve,500));
  await returnToVerifiedPart(data.job_id,data.part_id);
  active=null;
};
