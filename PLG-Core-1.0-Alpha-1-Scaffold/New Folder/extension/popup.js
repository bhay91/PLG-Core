...
async function apiFetch(path, options = {}) {
  ...
  throw lastError || new Error("PLG Core API is unavailable.");
}
...
async function refreshActiveVerification() {
  ...
  try {
    const { response } = await apiFetch("/api/active-verification");
    ...
    setStatus(
      "Could not connect to PLG Core. Confirm ./run.sh is still running, then reload the extension.",
      false
    );
  }
}
...
document.getElementById("capturePart").addEventListener("click", async () => {
  ...
  try {
    const { response } = await apiFetch("/api/capture-part", {
      method: "POST",
    ...
    setStatus("Could not send the captured part to PLG Core.", false);
  }
});
