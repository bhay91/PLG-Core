...
try:
    ...
    if 'supplier_additions' not in text:
        m=re.search(r'(\s+connection\.execute\(\s+"""\s+CREATE TABLE IF NOT EXISTS active_verification)',text)
        if not m: raise RuntimeError('Supplier schema anchor not found')
        text=text[:m.start()]+schema['supplier']+text[m.start():]
    if 'connector_columns =' not in text:
        i=text.find('        default_connectors = [')
    ...
    if 'quote_columns =' not in text:
        m=re.search(r'(\s+connection\.execute\(\s+"""\s+CREATE TABLE IF NOT EXISTS quote_items)',text)
        if not m: raise RuntimeError('Quote schema anchor not found')
        text=text[:m.start()]+schema['quote']+text[m.start():]
